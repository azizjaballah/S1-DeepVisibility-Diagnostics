<#
.SYNOPSIS
  SentinelOne Deep Visibility passive diagnostics for agents active in console but inactive in DV.

.DESCRIPTION
  This script performs READ-ONLY checks to diagnose Deep Visibility connectivity issues.
  It does not modify any settings or agent configuration.

.PARAMETER VerboseOutput
  Display detailed output to console in addition to JSON report.

.EXAMPLE
  .\S1-DeepVisibility-PassiveDiag.ps1 -VerboseOutput
#>
...
$OutFile   = Join-Path $OutputRoot "S1-DeepVisibilityDiag_$Timestamp.json"
...
function Get-S1DeepVisibilityConfig {
    ...
    try {
        ...
        if (Test-Path $regPath) {
            ...
            # Deep Visibility specific settings
            $dvSettings = @(
...
function Get-WindowsFirewallStatus {
    ...
    try {
        ...
        # Check for SentinelOne firewall rules
    ...
    try {
        ...
        foreach ($logPath in $logPaths) {
            if (Test-Path $logPath) {
                ...
                # Try to read last 50 lines for errors (if accessible)
                try {
...
# RUN TESTS
###############
if ($VerboseOutput) { Write-Host "Running diagnostics..." -ForegroundColor Yellow }
...
if ($null -eq $dvConfig.DVEnabled) {
    $issues += "WARNING: Deep Visibility configuration not found in registry"
}
...
if (-not $tlsResult.Success) {
    $issues += "CRITICAL: TLS 1.2 connection to $S1VisibilityUri failed"
}

# Proxy warnings
if ($proxyResult.WinHttpProxyRaw -notmatch "Direct access") {
    $issues += "INFO: Proxy is configured - verify S1 endpoints are allowed"
}
...
if ($VerboseOutput) {
    ...
    Write-Host "  TLS 1.2 HTTPS    : $($tlsResult.Success)"
