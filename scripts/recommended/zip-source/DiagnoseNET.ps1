...
.SYNOPSIS
  ...
  that are active in console but inactive in Deep Visibility.
...
.PARAMETER S1VisibilityHost
  The SentinelOne visibility host to test (for example: ioc-gw-prod-eu-1b.sentinelone.net).
...
[CmdletBinding()]
param(
    [Parameter(Mandatory=$true, HelpMessage="SentinelOne visibility host (for example: ioc-gw-prod-eu-1b.sentinelone.net)")]
...
# Fixed output file in TEMP (good for RemoteOps)
$Timestamp = (Get-Date).ToString("yyyyMMdd_HHmmss")
$OutFile   = Join-Path $env:TEMP "S1-DeepVisibilityDiag.json"

# Lightweight write test
try {
    '{"status":"init"}' | Out-File -FilePath $OutFile -Encoding UTF8 -Force
}
catch {
    Write-Host "CRITICAL: Cannot write to $OutFile : $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
function Get-S1AgentInfo {
    ...
    try {
        ...
        if ($processes) {
            ...
            $result.ProcessDetails = $processes | Select-Object Name, Id, StartTime, WorkingSet64 | ForEach-Object {
                [ordered]@{
                    Name      = $_.Name
                    PID       = $_.Id
...
#############################
# TLS TEST (RELAXED + 407 special-case)
#############################
function Test-S1Tls12 {
    ...
    $result = [ordered]@{
        ...
        Error              = $null
        Protocol           = "TLS1.2"
        HttpErrorTolerated = $false   # true when 2xx–4xx (except 407) but used as connectivity success
        ProxyAuthRequired  = $false   # true when HTTP 407 Proxy Authentication Required
    }
    ...
    try {
        ...
        if ($webEx.Response -and $webEx.Response.StatusCode) {
            ...
            elseif ($code -ge 200 -and $code -lt 500) {
                # 2xx–4xx (except 407) means TLS and HTTP path are OK
                $result.Success            = $true
...
if ($null -eq $dvConfig.DVEnabled) {
    $issues += "INFO: Deep Visibility configuration not in registry (may be managed only by console)"
}
...
if (-not $tlsResult.Success -and -not $tlsResult.HttpErrorTolerated) {
    if ($tlsResult.ProxyAuthRequired) {
        ...
        $issues += "INFO: TLS 1.2 test failed - this may be normal for Deep Visibility endpoints that reject non agent requests."
    }
}
elseif ($tlsResult.HttpErrorTolerated) {
    $issues += "INFO: TLS succeeded but HTTP status was $($tlsResult.StatusCode) - connectivity OK; request was not agent like."
}
...
$out = [ordered]@{
    Timestamp        = (Get-Date)
    DiagnosticVersion= "1.2-remoteops-safe"
    System           = $systemInfo
    Agent            = $agentInfo
...
$out | ConvertTo-Json -Depth 8 | Out-File -FilePath $OutFile -Encoding UTF8 -Force
...
if ($VerboseOutput) {
    ...
    Write-Host ""
    Write-Host "Deep Visibility Config:"
    ...
    Write-Host "  TLS 1.2 Test      : $($tlsResult.Success) StatusCode: $($tlsResult.StatusCode)"
    ...
    if ($issues.Count -gt 0) {
        ...
        Write-Host ""
        Write-Host "No issues detected - configuration appears normal." -ForegroundColor Green
    }

    Write-Host ""
    Write-Host "JSON report saved to:" -ForegroundColor Cyan
    Write-Host "  $OutFile"
    Write-Host ""
}
